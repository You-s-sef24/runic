...
import { Package, Clock, CheckCircle2, ArrowLeft, Hammer } from "lucide-react";
        ...
        <ProtectedRoutes>
            <main className="max-w-4xl mx-auto px-4 sm:px-6 py-10 sm:py-14">
                <div className="mb-8">
                    ...
                    >
                        <ArrowLeft size={14} className="rtl:rotate-180" /> {t("orders.backToShop")}
                    </Link>
                ...
                {!isLoading && !isError && sortedOrders.length === 0 && (
                    <div className="text-center py-16 border border-dashed border-zinc-200 dark:border-zinc-800 rounded-2xl bg-zinc-50/50 dark:bg-zinc-900/20">
                        ...
                        <h3 className="font-semibold text-zinc-800 dark:text-zinc-200 text-base">{t("orders.noOrdersTitle")}</h3>
                            ...
                            >
                                    ...
                                    <div className="flex flex-wrap gap-x-6 gap-y-1">
                                        <span>
                                            ...
                                            <strong className="text-zinc-800 dark:text-zinc-200">
                                                {new Date(order.createdAt * 1000).toLocaleDateString(lang === "ar" ? "ar-EG" : "en-US", {
                                                    month: "short",
                                                    day: "numeric",
                                                    year: "numeric",
                                                })}
                                        ...
                                        <span>
                                            {t("orders.id")} <strong className="text-zinc-800 dark:text-zinc-200">#{order.id}</strong>
                                        </span>
                                    </div>
                                ...
                                {order.items.length > 0 && (
                                    <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
                                        {order.items.map((item, i) => {
                                            const itemName = typeof item.name === "object" ? (lang === "en" ? item.name?.en : item.name?.ar) : item.name;
                                                ...
                                                <div key={`${order.id}-${i}`} className="p-5 flex items-start gap-4">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <h4 className="font-semibold text-zinc-800 dark:text-zinc-200 text-sm leading-tight hover:text-zinc-600 dark:hover:text-zinc-400 truncate">
                                                            <Link href={`/collection/${item.productId}`}>{itemName}</Link>
                                                        </h4>
                                                        <div className="flex items-center gap-3 text-xs text-zinc-500 dark:text-zinc-400 mt-2">
                                                            <span>
                                                                {t("orders.qty")} <strong className="text-zinc-700 dark:text-zinc-300">{item.quantity}</strong>
                                                            </span>
                                ...
                                <div className="px-5 py-3 border-t border-zinc-100 dark:border-zinc-800 bg-zinc-50/30 dark:bg-zinc-900/30 space-y-1">
                                    <p className="text-xs text-zinc-500 dark:text-zinc-400">
                                        {t("orders.shippingTo")} <span className="text-zinc-700 dark:text-zinc-300">{order.shippingAddress}</span>
                                    </p>
                                    {order.nails > 0 && (
                                        <p className="text-xs text-zinc-500 dark:text-zinc-400 flex items-center gap-1.5">
                                            <Hammer className="w-3 h-3" />
                                            {t("orders.nailsIncluded")} <span className="text-zinc-700 dark:text-zinc-300 font-medium">{order.nails}</span>
                                        </p>
                                    )}
                                </div>
                            </div>
