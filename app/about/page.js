    ...
    return (
        <main className="max-w-3xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
            ...
            <section className="space-y-5 mb-16 sm:mb-24 text-center sm:text-left">
                ...
                <h1 className="text-3xl sm:text-4xl font-bold text-zinc-900 tracking-tight leading-tight max-w-2xl">
                ...
                <div className="pt-2">
                    <Link href="/collection">
                        <Button className="bg-blue-900 hover:bg-blue-950 text-xs font-semibold uppercase tracking-wider gap-2 px-6 py-5 cursor-pointer shared-button-styles">
            ...
            <section className="py-12 border-t border-zinc-100 mb-16 sm:mb-24">
                ...
                <div className="grid sm:grid-cols-3 gap-6">
                    {VALUES.map((value, idx) => (
                        <div
                            key={idx}
                            className="p-6 border border-zinc-100 rounded-xl bg-zinc-50/30 flex flex-col items-start gap-4"
                        >
                            <div className="p-2.5 bg-white rounded-lg shadow-sm border border-zinc-100/60">
                                {value.icon}
                            </div>
                            <div>
                                <h3 className="font-semibold text-zinc-900 text-base mb-1.5">
                                    {value.title}
                                </h3>
                ...
                <Accordion type="single" collapsible className="w-full space-y-2">
                    {FAQS.map((faq, idx) => (
                        <AccordionItem
                            key={idx}
                            value={`faq-${idx}`}
                            className="border border-zinc-100 rounded-xl bg-white px-4 py-1 shadow-sm data-[state=open]:border-zinc-200 transition-all duration-200"
                        >
                            <AccordionTrigger className="text-zinc-800 hover:text-zinc-900 font-medium text-sm sm:text-base hover:no-underline py-4 text-left">
                                {faq.question}
                            </AccordionTrigger>
                            <AccordionContent className="text-zinc-500 text-xs sm:text-sm leading-relaxed pb-4">
                                {faq.answer}
                            </AccordionContent>
                        </AccordionItem>
                    ))}
